...
const membraneApi = require('../services/membrane-api');
const dreams = require('../services/dreams');
const server = require('../services/server');
const blog = require('../content/blog');
const usage = require('../services/usage');
const connectome = require('../services/connectome');
...
async function getDashboardData() {
  ...
  const [botsResult, aetheraStatus, aetheraHealth, membraneStatus, membraneHealth, blogStats, usageStats] = await Promise.allSettled([
    chapterx.listBots(),
    aethera.getStatus(),
    aethera.checkHealth(),
  ...
  const membraneData = membraneStatus.status === 'fulfilled' ? membraneStatus.value : { running: false };
  const membraneHealthData = membraneHealth.status === 'fulfilled' ? membraneHealth.value : { healthy: false };
  ...
  return {
    ...
    services: {
      ...
      membraneApi: {
        running: membraneData.running || false,
        health: membraneHealthData.healthy ? 'healthy' : 'unhealthy',
      },
    },
    dreams: {
      gpuConnected: dreams.getStatus().gpuConnected,
      status: dreams.getStatus().status,
      fps: dreams.getStatus().generation?.fps ?? null,
      overallHealth: dreams.getHealthWindows().overall,
    },
    blog: {
...
/**
 * Services: Aethera container + membrane-api + deployment slots status
 * @returns {Promise<Object>}
 */
async function getServicesData() {
  const [aetheraStatus, aetheraHealth, membraneStatus, membraneHealth, slots, bots] = await Promise.all([
    aethera.getStatus(),
    aethera.checkHealth(),
    membraneApi.getStatus(),
    membraneApi.checkHealth(),
...
async function getUsageData() {
  ...
  for (const period of periods) {
    ...
    // Calculate cache savings estimate (90% savings on cached reads at $3/MTok)
...
// ============================================================================
// CONNECTOME AGGREGATOR
// ============================================================================

/**
 * Connectome: repo git status + Nin services + runtime readouts + live deploy steps
 * @returns {Promise<Object>}
 */
async function getConnectomeData() {
