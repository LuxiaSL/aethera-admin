...
async function startDreams(options = {}) {
  ...
  try {
    ...
    if (shouldUseLifecycle) {
      ...
      // 4. Wait for ComfyUI to register its proxy URL
      // 5. Start DreamGen pod
      //
      ...
      console.log('  Goal: DreamGen pod exists (but will start later)');
      ...
      if (dreamgenPod) {
        ...
        if (dreamgenPod.desiredStatus === 'RUNNING') {
          ...
          console.log('  Stopping DreamGen (will restart after ComfyUI registers)...');
          try {
        ...
        try {
          ...
          // Stop the newly created pod - we'll start it after ComfyUI registers
          console.log('  Stopping newly created pod (will start after ComfyUI registers)...');
          try {
            await stopPod(newPod.id);
      ...
      // ==================== PHASE 3: Wait for ComfyUI Registration ====================
      console.log('\n[PHASE 3] Waiting for ComfyUI Registration');
      console.log('  ComfyUI must register its proxy URL before DreamGen can connect');
      results.steps.push({ step: 'wait_registration', status: 'in_progress' });
      
      let registered = false;
      let registryEndpoint = null;
      ...
      while (!registered && (Date.now() - startTime) < maxWaitTime) {
        ...
        if (registryStatus.registered) {
          registered = true;
          registryEndpoint = registryStatus.endpoint;
          console.log(`  ✓ ComfyUI registered after ${pollCount} polls`);
          console.log(`    URL: ${registryEndpoint?.url}`);
          break;
        }
      ...
      if (dreamgenStarted) {
        ...
        results.steps[3].podId = dreamgenPod.id;
