...
async function startPod(podId) {
  ...
  const data = await graphqlRequest(mutation, { podId });
  console.log(`Pod ${podId} resume requested:`, data.podResume);
  ...
  return {
    success: true,
    pod: data.podResume,
    message: `Pod ${podId} resume requested`,
  };
}
...
async function stopPod(podId) {
  ...
  const data = await graphqlRequest(mutation, { podId });
  console.log(`Pod ${podId} stop requested:`, data.podStop);
