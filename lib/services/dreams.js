...
async function startDreams(options = {}) {
  ...
  try {
    ...
    if (shouldUseLifecycle) {
      ...
      // Step 0: Clear any stale ComfyUI registration first
      console.log('Step 0: Clearing any stale ComfyUI registration...');
      try {
        await unregisterComfyUI();
        console.log('Cleared stale registration');
      } catch (e) {
        console.log('No stale registration to clear (expected)');
      }
      ...
      const comfyuiResult = await ensurePod('comfyui', {
        verifyOnly: false,  // Actually start and keep running
        leaveRunning: true,
        secretOverrides: {
      ...
      console.log(`ComfyUI pod ${comfyuiResult.action}: ${comfyuiResult.podId}`);
      
      if (comfyuiResult.warnings.length > 0) {
        results.warnings.push(...comfyuiResult.warnings.map(w => `ComfyUI: ${w}`));
      }
      
      // Step 2: Wait for ComfyUI to fully start and register with VPS
      // This is CRITICAL - DreamGen cannot start until ComfyUI has registered its IP
      console.log('========================================');
      console.log('Step 2: Waiting for ComfyUI registration...');
      console.log('DreamGen will NOT start until ComfyUI registers its IP with VPS');
      console.log('========================================');
      ...
      while (!registered && (Date.now() - startTime) < maxWaitTime) {
        ...
        console.log(`Registration poll #${pollCount}: registered=${registryStatus.registered}`);
      ...
      if (!registered) {
        ...
        console.warn('========================================');
        console.warn('WARNING: ComfyUI registration TIMEOUT after 5 minutes!');
        console.warn('Starting DreamGen anyway, but it may fail to connect');
        console.warn('========================================');
      }
      
      // Step 3: NOW start DreamGen pod (only after ComfyUI is registered)
      console.log('========================================');
      console.log('Step 3: NOW starting DreamGen pod...');
      console.log('========================================');
      ...
      console.log(`DreamGen pod ${dreamgenResult.action}: ${dreamgenResult.podId}`);
      ...
      console.log('========================================');
      console.log('Dreams system startup sequence COMPLETE');
      console.log('========================================');
