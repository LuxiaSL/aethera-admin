...
const MODEL_PRICING = {
  ...
  'claude-haiku-4-5-20251001': { input: 0.0008, output: 0.004, cacheWriteMult: 1.25, cacheReadMult: 0.1 },
  ...
  'openai/o3': { input: 0.01, output: 0.04, cacheWriteMult: 0, cacheReadMult: 0.5 },
  ...
  // Moonshot Kimi (via OpenRouter - estimates)
  'moonshotai/kimi-k2': { input: 0.0006, output: 0.0024, cacheWriteMult: 0, cacheReadMult: 0 },
...
function calculateCost(model, inputTokens, outputTokens, cacheReadTokens = 0, cacheWriteTokens = 0) {
  ...
  // Calculate costs
  // Uncached input = inputTokens - cacheReadTokens (those were read from cache)
...
function getBotUsage(botName, options = {}) {
  ...
  const totalCacheable = result.total_input_tokens || 0;
  const cacheHitRatio = totalCacheable > 0 
    ? (result.total_cache_read_tokens || 0) / totalCacheable 
    : 0;
