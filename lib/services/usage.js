 ...
 * 
 * Token semantics from Anthropic API:
 * - inputTokens: tokens sent as input (NOT including cache reads)
 * - cacheReadTokens: tokens read from cache (billed at cacheReadMult * input price)
 * - cacheWriteTokens: tokens written to cache (billed at cacheWriteMult * input price)
...
function calculateCost(model, inputTokens, outputTokens, cacheReadTokens = 0, cacheWriteTokens = 0) {
  ...
  // Calculate costs
  // inputTokens is ALREADY the uncached input (API reports them separately)
...
function getBotUsage(botName, options = {}) {
  ...
  // Total input = uncached input + cached input (cache reads)
  const totalInput = (result.total_input_tokens || 0) + (result.total_cache_read_tokens || 0);
  const cacheHitRatio = totalInput > 0 
    ? (result.total_cache_read_tokens || 0) / totalInput 
    : 0;
...
function getAllBotsUsage(options = {}) {
  ...
  return results.map(r => {
    const totalInput = (r.total_input_tokens || 0) + (r.total_cache_read_tokens || 0);
    return {
      ...r,
      cache_hit_ratio: totalInput > 0 
        ? (r.total_cache_read_tokens || 0) / totalInput 
        : 0,
    };
  });
}
...
function getTotals(options = {}) {
  ...
  // Calculate cache hit ratio: cache reads / total input (uncached + cached)
  const totalInput = (result.total_input_tokens || 0) + (result.total_cache_read_tokens || 0);
