 ...
 * 
 * Token semantics from trace data:
 * - inputTokens: TOTAL tokens sent as input (includes both cached and uncached)
 * - cacheReadTokens: tokens read from cache (a SUBSET of inputTokens)
 * - cacheWriteTokens: tokens written to cache
 * - outputTokens: tokens generated by the model
 * 
 * Cost calculation:
 * - Uncached input = inputTokens - cacheReadTokens (billed at full input price)
 * - Cache reads = cacheReadTokens (billed at cacheReadMult * input price)
 * - Cache writes = cacheWriteTokens (billed at cacheWriteMult * input price)
 * - Output = outputTokens (billed at output price)
 */
function calculateCost(model, inputTokens, outputTokens, cacheReadTokens = 0, cacheWriteTokens = 0) {
  ...
  // Calculate costs
  // Uncached input = total input minus the portion read from cache
