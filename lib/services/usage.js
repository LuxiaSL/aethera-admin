...
function getBotUsage(botName, options = {}) {
  ...
  const stmt = db.prepare(`
    SELECT 
      ...
      SUM(CASE WHEN cache_read_tokens > 0 THEN 1 ELSE 0 END) as requests_with_cache_hit,
  ...
  // Cache hit ratio = % of requests that read from cache
  const cacheHitRatio = result.request_count > 0 
    ? (result.requests_with_cache_hit || 0) / result.request_count 
    : 0;
...
function getAllBotsUsage(options = {}) {
  ...
  return results.map(r => ({
    ...r,
    cache_hit_ratio: r.request_count > 0 
      ? (r.requests_with_cache_hit || 0) / r.request_count 
      : 0,
  }));
}
...
function getTotals(options = {}) {
  ...
  return {
    ...result,
    cache_hit_ratio: result.total_requests > 0 
      ? (result.requests_with_cache_hit || 0) / result.total_requests 
      : 0,
  };
}
