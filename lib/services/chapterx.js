...
async function gitDiff(slot) {
  ...
  try {
    ...
    const files = statusLines.map(line => {
      ...
      // Git porcelain format: "XY filename" - status is 2 chars, then space, then path
      // Use substring(3) but handle edge cases where there might be extra whitespace
      const filePath = line.substring(3).replace(/^\s+/, '');
