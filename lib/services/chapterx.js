    ...
    if (dep.repoUrl && dep.branch) {
      try {
        ...
        const result = await runCmdFull(
          `git ls-remote ${dep.repoUrl}.git refs/heads/${dep.branch} 2>/dev/null`
        );
        
        if (result.code === 0 && result.stdout.trim()) {
          const match = result.stdout.match(/^([a-f0-9]{40})/);
          if (match) {
            dep.latestCommit = match[1].slice(0, 7); // Store short hash for comparison
            // Compare short hashes (installed is already short from package-lock)
