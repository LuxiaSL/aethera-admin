...
async function getBotStatus(botName) {
  // Check both slots
  const mainService = systemd.getServiceName(botName, 'main');
  const devService = systemd.getServiceName(botName, 'dev');
  
  const mainStatus = await systemd.getServiceStatus(mainService);
  const devStatus = await systemd.getServiceStatus(devService);
  
  const running = mainStatus.running || devStatus.running;
  const activeSlot = mainStatus.running ? 'main' : (devStatus.running ? 'dev' : null);
  ...
  return {
    ...
    mainStatus,
    devStatus,
    pid: mainStatus.pid || devStatus.pid || null,
    startedAt: mainStatus.startedAt || devStatus.startedAt || null,
  };
}
...
async function startBot(botName, slot = 'main') {
  // Validate slot
  if (!['main', 'dev'].includes(slot)) {
    throw new Error(`Invalid slot: ${slot}. Must be 'main' or 'dev'.`);
  }
...
function getSlotInfo() {
  ...
  for (const [name, slotPath] of Object.entries(config.CHAPTERX_SLOTS)) {
 ...
 * @param {string} slot - Slot name (main/dev)
 * @returns {Promise<Object>} Git status
 */
async function getSlotGitStatus(slot) {
  const slotPath = config.CHAPTERX_SLOTS[slot];
...
async function getAllSlotsStatus() {
  ...
  for (const slotName of Object.keys(config.CHAPTERX_SLOTS)) {
