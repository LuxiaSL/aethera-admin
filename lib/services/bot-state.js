 ...
 * NOTE: Does NOT change preferredSlot - that's only set by user action or startBot
 * @param {string} botName - Bot name
...
function recordBotStop(botName, slot) {
  ...
  // Record which slot was stopped, but DON'T change preferredSlot
  // preferredSlot is only set by explicit user action or when starting a bot
  if (slot) {
