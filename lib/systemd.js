...
function generateServiceFile(botName, slot) {
  const chapterxPath = config.CHAPTERX_SLOTS[slot] || config.CHAPTERX_SLOTS.main;
  
  // ChapterX is a Node.js/TypeScript project
  // It uses EMS_PATH + BOT_NAME environment variables to find config
  // Command: npx --yes tsx src/main.ts
  
  // Use bash -c to ensure proper PATH resolution for npx/node
  // This mirrors how borgs-admin launches bots (they use screen which inherits shell env)
  const execCmd = `/bin/bash -c 'cd "${chapterxPath}" && EMS_PATH="${config.BOTS_PATH}" BOT_NAME="${botName}" npx --yes tsx src/main.ts'`;
...
# Ensure node/npm are in PATH (adjust if using nvm or other version managers)
Environment=PATH=/usr/local/bin:/usr/bin:/bin
Environment=NODE_ENV=production

# Run ChapterX with tsx (TypeScript executor)
# Uses bash to ensure proper environment and PATH resolution
ExecStart=${execCmd}
  ...
  try {
    ...
    for (const line of lines) {
      ...
      if (match) {
        ...
        const botMatch = serviceName.match(/^chapterx-(.+)-(main|dev)$/);
        if (botMatch) {
          services.push({
            serviceName,
            botName: botMatch[1],
